...
int SetupMQTTClientConnection(Network* net, MQTTClient* mqtt_client, opts_struct* mqtt_options, MQTTPacket_connectData* mqtt_datapacket, uint8_t *ssn_server_ip, 
    ...
    SetupMQTTOptions(mqtt_options, cliendId_node_exclusive_channel, QOS1, 1, ssn_server_ip);
    ...
    while (1) {
        ...
        if (ConnectNetwork(net, ssn_server_ip, MQTT_Port) != SOCK_OK) {
            ...
            if (rc == SUCCESSS) {
                ...
                rc = MQTTSubscribe(mqtt_client, cliendId_node_exclusive_channel, mqtt_options->qos, messageArrivedoverMQTT);
                printf("[MQTT] Subscribing to Node Exclusive Channel: %s\n", cliendId_node_exclusive_channel);
                if (rc == SUCCESSS) {
