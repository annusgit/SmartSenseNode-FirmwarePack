...
void RunSystemTests(int temperature_min_threshold, int temperature_max_threshold) {
	...
	uint8_t test_temperature_bytes[2] = {0};
...
#ifdef TH_AM2320
    int8_t read_status = sample_Temperature_Humidity_bytes_using_AM2320(test_temperature_bytes, test_relative_humidity_bytes);
    // TODO: Add gradient based temperature and humidity change check
    bool temperature_sensor_test_passed = true;
    if (read_status==SENSOR_READ_ERROR) {
        temperature_sensor_test_passed = false;
	...
	if (temperature_sensor_test_passed) {
	...
	if (object_celcius_temperature > temperature_min_threshold && object_celcius_temperature < temperature_max_threshold){
		test_abnormal_activity = NORMAL_AMBIENT_CONDITION;
	} else {
		test_abnormal_activity = ABNORMAL_AMBIENT_CONDITION;
	}
	if (test_abnormal_activity == NORMAL_ACTIVITY_STATE) {
		printf("[LOG] TEMPERATURE SENSOR TEST Successful.\n");
	} else {
		printf("[LOG] TEMPERATURE SENSOR TEST FAILED DUE TO ABNORMAL CONDITION...\n");
	}
