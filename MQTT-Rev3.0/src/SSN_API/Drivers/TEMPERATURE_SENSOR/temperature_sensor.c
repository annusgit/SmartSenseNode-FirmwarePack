...
bool DHT22_OW_Read_Temp_and_Humidity() {
    ...
    SetPinForMicroseconds(IOPORT_A, BIT_2, 25);      // bring it back to high/idle
    ...
    uint8_t i; for(i=0; i<40; i++) {
        ...
        uint16_t j; for(j=0; j<counts_used; j++) {
            printf("%d ", sample_counter[j]);
        }
        uint8_t k; for(k=0; k<5; k++) {
...
int8_t sample_Temperature_Humidity_bytes_using_AM2320(uint8_t* temperature_bytes, uint8_t* relative_humidity_bytes) {
    ...
    if(!CRC_check()) return SENSOR_CRC_ERROR;
