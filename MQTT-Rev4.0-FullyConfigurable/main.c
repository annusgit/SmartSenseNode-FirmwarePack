...
int main() {
    ...
    SetupConnectionWithStaticIP(SSN_MAC_ADDRESS, SSN_STATIC_IP, SSN_SUBNET_MASK, SSN_GATEWAY_ADDRESS, SSN_DNS_ADDRESS);
#endif	 
#ifdef USE_DNS
	...
	// Get SSN network configurations for SSN or pick from EEPROM if already assigned
    SSN_GET_NETWORK_CONFIG(MQTTallowedfailureCount);
	// Get SSN sensor configurations for SSN or pick from EEPROM if already assigned
    SSN_GET_SENSOR_CONFIG(MQTTallowedfailureCount);
