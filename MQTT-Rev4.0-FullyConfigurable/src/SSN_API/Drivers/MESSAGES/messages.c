...
uint8_t is_Valid_CONFIG(uint8_t* config_array) {
    uint8_t valid = EEPROM_SENSOR_CONFIG_SIZE;
...
uint8_t construct_get_network_configuration_message(uint8_t* message_array, uint8_t* node_id) {
	uint8_t count = 0;
	...
	/* Send the MESSAGE ID */
	message_array[count++] = GET_NETWORK_CONFIG_MESSAGE_ID;

	// return how many bytes the message is
	return count;
}

uint8_t construct_ack_network_configuration_message(uint8_t* message_array, uint8_t* node_id, uint8_t* received_configs) {
	uint8_t count = 0;
	...
	/* Send the MESSAGE ID */
	message_array[count++] = ACK_NETWORK_CONFIG_MESSAGE_ID;

	/* Attach the received configurations with this message */
	for (count; count < (EEPROM_NETWORK_CONFIG_MIN_SIZE + 7 + received_configs[17]); count++) {
...
uint8_t construct_get_sensor_configuration_message(uint8_t* message_array, uint8_t* node_id) {
	uint8_t count = 0;
	...
	/* Send the MESSAGE ID */
	message_array[count++] = GET_SENSOR_CONFIG_MESSAGE_ID;

	// return how many bytes the message is
	return count;
}

uint8_t construct_ack_sensor_configuration_message(uint8_t* message_array, uint8_t* node_id, uint8_t* received_configs) {
	uint8_t count = 0;
	...
	/* Send the MESSAGE ID */
	message_array[count++] = ACK_SENSOR_CONFIG_MESSAGE_ID;

	/* Attach the received configurations with this message */
	for (count; count < EEPROM_SENSOR_CONFIG_SIZE + 7; count++) {
		message_array[count] = received_configs[count - 7];
	}
...
uint8_t decipher_received_message(uint8_t* message, uint8_t* params) {
	...
	switch (message_id) {
		...
		case SET_NETWORK_CONFIG_MESSAGE_ID:
			for (i = 0; i < (EEPROM_NETWORK_CONFIG_MIN_SIZE + message[17]); i++)
				params[i] = message[i + 7];
			break;
			
		// Set CONFIG received?
		case SET_SENSOR_CONFIG_MESSAGE_ID:
			// min size is if no DNS address is sent with the message
			// in case it is sent, byte 17 holds its variable size 
			for (i = 0; i < EEPROM_SENSOR_CONFIG_SIZE; i++)
				params[i] = message[i + 7];
			break;
