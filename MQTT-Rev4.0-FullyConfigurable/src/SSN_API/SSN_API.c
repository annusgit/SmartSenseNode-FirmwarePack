...
uint8_t SSN_GATEWAY_ADDRESS[4] = {192, 168, 0, 1};
...
uint8_t SSN_CONFIG[EEPROM_SENSOR_CONFIG_SIZE];
...
void SSN_GET_NETWORK_CONFIG(uint8_t allowed_mqtt_failure_counts) {
	// Find network configurations in EEPROM
	SSN_PREV_STATE = SSN_CURRENT_STATE;
	SSN_CURRENT_STATE = FindNetworkConfigurationsInFlashMemory();
	uint16_t SendAfter = 0, mqtt_failed_count = 0;
	while (SSN_CURRENT_STATE == NO_NETWORK_CONFIG_STATE) {
		SSN_PREV_STATE = SSN_CURRENT_STATE;
		SSN_CURRENT_STATE = NO_NETWORK_CONFIG_STATE;
		if (SSN_PREV_STATE != SSN_CURRENT_STATE) {
			Clear_LED_INDICATOR();
		}
		if (CONFIG_received) { break; }
		// request a Configuration after every 5 seconds
		if (SendAfter % 50 == 0) {
			message_publish_status = Send_GETNETWORKCONFIG_Message(SSN_MAC_ADDRESS);
			if (message_publish_status != SUCCESSS) {
				message_publish_status = FAILURE;
                mqtt_failed_count++;
                printf("[ERROR] Message Publication to MQTT Broker Failed (Count = %d/%d)\n", mqtt_failed_count, allowed_mqtt_failure_counts);
...
void SSN_GET_SENSOR_CONFIG(uint8_t allowed_mqtt_failure_counts) {
	// Find sensor configurations in EEPROM
	SSN_PREV_STATE = SSN_CURRENT_STATE;
	...
	uint16_t SendAfter = 0, mqtt_failed_count = 0;
	while (SSN_CURRENT_STATE == NO_SENSOR_CONFIG_STATE) {
		SSN_PREV_STATE = SSN_CURRENT_STATE;
		SSN_CURRENT_STATE = NO_SENSOR_CONFIG_STATE;
		if (SSN_PREV_STATE != SSN_CURRENT_STATE) {
			Clear_LED_INDICATOR();
		}
		if (CONFIG_received) { break; }
		// request a Configuration after every 5 seconds
		if (SendAfter % 50 == 0) {
			message_publish_status = Send_GETSENSORCONFIG_Message(SSN_MAC_ADDRESS);
			if (message_publish_status != SUCCESSS) {
				message_publish_status = FAILURE;
                mqtt_failed_count++;
                printf("[ERROR] Message Publication to MQTT Broker Failed (Count = %d/%d)\n", mqtt_failed_count, allowed_mqtt_failure_counts);
...
int SSN_Check_Connection_And_Reconnect(int return_code) {
	...
	if (return_code != SUCCESSS) {
		...
		rc = SetupMQTTClientConnection(&MQTT_Network, &Client_MQTT, &MQTTOptions, SSN_SERVER_IP, NodeExclusiveChannel, SSN_RECEIVE_ASYNC_MESSAGE_OVER_MQTT);
		// printf("rc %d\n", rc);
		return rc;
	}
	return SUCCESSS;
	...
	if (MQTTOptions.showtopics) {
		...
		*(testbuffer + (int) (message->payloadlen) + 1) = '\n';
		...
		switch (received_message_id) {
			...
			case SET_SENSOR_CONFIG_MESSAGE_ID:
				// write the new config to designated location in EEPROM
				EEPROM_Write_Array(EEPROM_SENSOR_CONFIG_BLOCK, EEPROM_SENSOR_CONFIG_LOC, params, EEPROM_SENSOR_CONFIG_SIZE);
				// Copy received configurations to the SSN_CONFIG array
				int i; for (i = 0; i < EEPROM_SENSOR_CONFIG_SIZE; i++) {
				...
				printf("[LOG] Received New Sensors Configuration from SSN Server: \n"
            ...
            case RETRIEVE_CURRENT_CONFIG_MESSAGE_ID:
				printf("<- RETRIEVE SENSOR CONFIG MESSAGE RECEIVED\n");
...
void SSN_REQUEST_Time_of_Day_AFTER_N_SECONDS(uint32_t seconds, uint8_t allowed_mqtt_failure_counts) {
	...
	if ((ssn_uptime_in_seconds % seconds) == 0 && ssn_uptime_in_seconds != 0) {
		...
		SSN_GET_TIMEOFDAY(allowed_mqtt_failure_counts);
	}
}
