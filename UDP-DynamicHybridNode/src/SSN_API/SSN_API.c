...
/** SSN Server PORT */
uint16_t SSN_SERVER_PORT = 25000;

/** Static IP Assignment */
//uint8_t SSN_STATIC_IP[4]		= {172, 16, 0, 56};
//uint8_t SSN_SUBNET_MASK[4]		= {255, 255, 255, 192};
//uint8_t SSN_GATWAY_ADDRESS[4]	= {172, 16, 0, 62};
uint8_t SSN_STATIC_IP[4]		= {192, 168, 0, 167};
...
void SSN_GET_CONFIG() {
	...
	while (SSN_CURRENT_STATE == NO_CONFIG_STATE) {
		...
		// Try to receive a message every 100 milliseconds
		if (Receive_CONFIG(SSN_UDP_SOCKET, SSN_MAC_ADDRESS, SSN_SERVER_IP, SSN_SERVER_PORT, SSN_CONFIG, &SSN_REPORT_INTERVAL, &TEMPERATURE_MIN_THRESHOLD, &TEMPERATURE_MAX_THRESHOLD, &RELATIVE_HUMIDITY_MIN_THRESHOLD,
			&RELATIVE_HUMIDITY_MAX_THRESHOLD, SSN_CURRENT_SENSOR_RATINGS, SSN_CURRENT_SENSOR_THRESHOLDS, SSN_CURRENT_SENSOR_MAXLOADS, SSN_CURRENT_SENSOR_VOLTAGE_SCALARS, Machine_status)) {
			break;
		}
		// Give LED indication every second
		if (SendAfter % 10 == 0) {
...
void SSN_GET_TIMEOFDAY() {
	...
	while (1) {
		...
		// Try to receive a message every 100 milliseconds
		if (Receive_TimeOfDay(SSN_UDP_SOCKET, SSN_MAC_ADDRESS, SSN_SERVER_IP, SSN_SERVER_PORT)) {
...
void SSN_RECEIVE_ASYNC_MESSAGE_AND_ROUTE_OR_CONSUME() {
	...
	Receive_CONFIG(SSN_UDP_SOCKET, SSN_MAC_ADDRESS, SSN_SERVER_IP, SSN_SERVER_PORT, SSN_CONFIG, &SSN_REPORT_INTERVAL, &TEMPERATURE_MIN_THRESHOLD, &TEMPERATURE_MAX_THRESHOLD, &RELATIVE_HUMIDITY_MIN_THRESHOLD,
		&RELATIVE_HUMIDITY_MAX_THRESHOLD, SSN_CURRENT_SENSOR_RATINGS, SSN_CURRENT_SENSOR_THRESHOLDS, SSN_CURRENT_SENSOR_MAXLOADS, SSN_CURRENT_SENSOR_VOLTAGE_SCALARS, Machine_status);
	Receive_TimeOfDay(SSN_UDP_SOCKET, SSN_MAC_ADDRESS, SSN_SERVER_IP, SSN_SERVER_PORT);
}
...
void SSN_GET_AMBIENT_CONDITION(uint8_t temp_min, uint8_t temp_max, uint8_t rh_min, uint8_t rh_max) {
	...
	abnormal_activity = ambient_condition_status(temp_min, temp_max, rh_min, rh_max);
